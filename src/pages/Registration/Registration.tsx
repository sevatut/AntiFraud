...
export default function Registration() {
  ...
  const {
    ...
    const onSubmit = (data: RegistrationFormData) => {
      if (!registerUser(data.login, data.password)) {
  ...
  return (
    <div className="p-40 flex justify-center h-screen bg-[#E9EFF2]">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-center border border-[#E3E4E5] bg-white w-min px-10 py-10 rounded-xl">
        <h2 className="text-5xl mb-15 text-[#4E80D1]">{t("registration.header")}</h2>
        <label className="flex flex-col mb-5">
            <p className="text-3xl font-medium mb-5">{t("registration.login")}</p>
        ...
        <label className="flex flex-col mb-12">
            <p className="text-3xl font-medium mb-5">{t("registration.password")}</p>
        ...
        <button type="submit" className="w-48 p-3 border text-lg border-[#86B4E1] text-[#4E80D1] rounded-xs cursor-pointer
                            hover:bg-[#1C7FDA] hover:text-white mb-10">{t("registration.button")}</button>

        <p className="text-xl">{t("registration.question")} <Link to="/login" className="text-[#1d59b9] cursor-pointer">{t("registration.link")}</Link></p>
      </form>
    </div>
  )
}